...
export type ProjectStatusType =
  keyof typeof import('@prisma/client').ProjectStatus;
...
> = {
  ACTIVE: {
    ...
    label: 'Actif',
    description:
      "Projet en cours d'utilisation, accessible pour consultation et modification",
    color: '#10B981', // Green-500
    allowedTransitions: [
      'ARCHIVED' as import('@prisma/client').ProjectStatus,
      'DELETED' as import('@prisma/client').ProjectStatus,
    ],
  },
  ARCHIVED: {
    ...
    allowedTransitions: [
      'ACTIVE' as import('@prisma/client').ProjectStatus,
      'DELETED' as import('@prisma/client').ProjectStatus,
    ],
  },
  DELETED: {
...
export function isValidProjectStatus(
  status: string,
): status is import('@prisma/client').ProjectStatus {
 ...
 */
function createSecureMetadataCopy(
  ...
  return {
    status:
      metadata.status || ('ACTIVE' as import('@prisma/client').ProjectStatus),
    label:
      typeof metadata.label === 'string' ? metadata.label : 'Statut inconnu',
    description:
 ...
 */
export const ALL_PROJECT_STATUSES = Object.values(PROJECT_STATUS_METADATA).map(
 ...
 */
export const USER_ACCESSIBLE_STATUSES = ALL_PROJECT_STATUSES.filter(
  (status) => !isDeletedStatus(status),
);
