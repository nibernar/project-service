...
export class UnauthorizedAccessException extends ForbiddenException {
  ...
  constructor(
    ...
    const publicMessage = UnauthorizedAccessException.generatePublicMessage(
      resourceType,
      action,
    );
   ...
   */
  private static generatePublicMessage(
    resourceType?: string,
    action?: string,
  ): string {
  ...
  isSuspiciousActivity(recentAttempts: number = 0): boolean {
    ...
    if (
      this.action &&
      ['delete', 'admin', 'modify'].includes(this.action.toLowerCase())
    ) {
      return true;
    }
