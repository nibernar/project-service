...
export class ProjectOwnerGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      const ownership = await this.checkProjectOwnership(
        projectId,
        user.id,
        options,
      );
  ...
  private extractUser(request: FastifyRequest): User {
    try {
      ...
      if (!user) {
        this.logger.error(
          'User not found in request context - AuthGuard may not be applied',
        );
  ...
  private async checkProjectOwnership(
    ...
    try {
      ...
      if (!project) {
        ...
        const projectExists = await this.databaseService.executeWithRetry(
          async () => {
            return await this.databaseService.project.findFirst({
      ...
      this.logger.error(
        `Database error during ownership check for project ${projectId}`,
        error,
      );
      throw new InternalServerErrorException(
  ...
  private async auditOwnershipCheck(
    ...
    try {
      ...
      if (success) {
        this.logger.log(
          `✅ Ownership verified for project ${projectId} by user ${user?.email}`,
          auditData,
        );
      } else {
        this.logger.warn(
          `❌ Ownership check failed for project ${projectId}: ${error}`,
          auditData,
        );
      }
  ...
  private handleOwnershipError(error: any, context: string): never {
    ...
    // Erreurs inattendues
    this.logger.error(
      `Unexpected ownership verification error in ${context}`,
      error,
    );
