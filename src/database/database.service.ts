  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  constructor(
    ...
    if (!dbConfig.url) {
      throw new Error(
        'DATABASE_URL is required but not provided in database configuration',
      );
    }
    ...
    const prismaOptions: Prisma.PrismaClientOptions = {
      ...
      log: dbConfig.logging.enabled
        ? dbConfig.logging.level.map((level) => ({
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.logger.debug(
      `Database config: Max connections: ${this.dbConfig.maxConnections}, SSL: ${typeof this.dbConfig.ssl === 'boolean' ? this.dbConfig.ssl : this.dbConfig.ssl.enabled}`,
    );
    ...
    try {
      ...
      throw new Error(
        `Failed to connect to database after ${this.dbConfig.retries.maxRetries} attempts`,
      );
    }
  }
  ...
  private async runMigrations(): Promise<void> {
    try {
      ...
      this.logger.log(
        'Migrations completed (handled by Prisma CLI in production)',
      );
    } catch (error) {
  ...
  private async connectWithRetry(): Promise<void> {
    const maxRetries = this.dbConfig.retries.enabled
      ? this.dbConfig.retries.maxRetries
      : 1;
    let lastError: Error | null = null;
      ...
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
          setTimeout(
            ...
            this.dbConfig.connectionTimeout,
          );
        });
  ...
  private handleConnectionError(error: Error): void {
    ...
    if (error.message.includes('ECONNREFUSED')) {
      ...
      this.logger.error(
        'Database connection timeout - check network connectivity',
      );
  ...
  private setupQueryLogging(): void {
    if (
      this.dbConfig.logging.enabled &&
      this.dbConfig.logging.level.includes('query')
    ) {
  ...
  private logDatabaseMetrics(): void {
    ...
    const slowQueryRatio =
      performance.totalQueries > 0
        ? performance.slowQueries / performance.totalQueries
        : 0;

    if (slowQueryRatio > 0.1) {
      // Plus de 10% de requêtes lentes
      this.logger.warn(
        `High slow query ratio: ${Math.round(slowQueryRatio * 100)}%`,
      );
    }
  }
}
