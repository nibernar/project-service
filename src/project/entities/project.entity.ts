...
export class ProjectEntity {
  ...
  @Length(1, 100, { message: 'Le nom du projet doit contenir entre 1 et 100 caractères' })
  @Transform(({ value }) => {
  ...
  @Length(0, 1000, { message: 'La description ne peut pas dépasser 1000 caractères' })
  @Transform(({ value }) => {
  ...
  @ApiProperty({
    ...
    example: 'Je souhaite créer une application de gestion des ressources humaines avec authentification, gestion des employés, calcul de paie et reporting.',
    minLength: 10,
    maxLength: 5000,
  })
  ...
  @Length(10, 5000, { message: 'Le prompt initial doit contenir entre 10 et 5000 caractères' })
  @Transform(({ value }) => {
  ...
  @IsEnum(ProjectStatus, { message: 'Le statut doit être une valeur valide (ACTIVE, ARCHIVED, DELETED)' })
  @Expose()
  status: ProjectStatus;
  ...
  @IsArray({ message: 'Les identifiants de fichiers uploadés doivent former un tableau' })
  @IsUUID(4, { each: true, message: 'Chaque identifiant de fichier uploadé doit être un UUID v4 valide' })
  @Transform(({ value }) => {
  ...
  @IsArray({ message: 'Les identifiants de fichiers générés doivent former un tableau' })
  @IsUUID(4, { each: true, message: 'Chaque identifiant de fichier généré doit être un UUID v4 valide' })
  @Transform(({ value }) => {
  ...
  @IsUUID(4, { message: 'L\'identifiant du propriétaire doit être un UUID v4 valide' })
  ...
  addGeneratedFiles(fileIds: string[]): void {
    ...
    const newFileIds = fileIds.filter(id => !this.generatedFileIds.includes(id));
  ...
  canTransitionTo(newStatus: ProjectStatus): boolean {
    ...
    switch (this.status) {
      case ProjectStatus.ACTIVE:
        return newStatus === ProjectStatus.ARCHIVED || newStatus === ProjectStatus.DELETED;
      case ProjectStatus.ARCHIVED:
        return newStatus === ProjectStatus.ACTIVE || newStatus === ProjectStatus.DELETED;
   ...
   */
  toString(): string {
    const filesInfo = this.getFileCount().total > 0 
  ...
  static create(data: CreateProjectData, ownerId: string): ProjectEntity {
    ...
    if (data.uploadedFileIds && !ProjectEntity.validateFileIds(data.uploadedFileIds)) {
    ...
    project.status = ProjectStatus.ACTIVE;
    project.uploadedFileIds = data.uploadedFileIds ? [...data.uploadedFileIds] : [];
    project.generatedFileIds = [];
  ...
  static update(project: ProjectEntity, data: UpdateProjectData): void {
    ...
    if (data.uploadedFileIds !== undefined && !ProjectEntity.validateFileIds(data.uploadedFileIds)) {
      throw new Error('Invalid uploaded file IDs');
    }
    if (data.generatedFileIds !== undefined && !ProjectEntity.validateFileIds(data.generatedFileIds)) {
      throw new Error('Invalid generated file IDs');
    }
    if (data.status !== undefined && !project.canTransitionTo(data.status)) {
      throw new Error(`Invalid status transition from ${project.status} to ${data.status}`);
    }
