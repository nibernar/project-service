...
export class ProjectController {
  ...
  @ApiOperation({ 
    ...
    description: 'Creates a new project and triggers the document generation workflow'
  })
  @ApiResponse({ 
    status: 201, 
  ...
  async create(
    ...
    this.logger.log(`Creating project "${createDto.name}" for user ${user.email}`, {
      operation: 'create',
      userId: user.id,
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieves a paginated list of user projects with optional filtering'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiQuery({ name: 'hasGeneratedFiles', required: false, type: Boolean, description: 'Filter projects with generated files' })
  @ApiQuery({ name: 'hasStatistics', required: false, type: Boolean, description: 'Filter projects with statistics' })
  @ApiQuery({ name: 'createdAfter', required: false, type: String, description: 'Filter projects created after date' })
  @ApiQuery({ name: 'createdBefore', required: false, type: String, description: 'Filter projects created before date' })
  @ApiQuery({ name: 'orderBy', required: false, enum: ['createdAt', 'updatedAt', 'name'], description: 'Sort field' })
  ...
  async findAll(
    ...
    try {
      ...
      const result = await this.projectService.findAll(user.id, pagination, searchOptions);
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieves detailed project information including statistics if available'
  })
  @ApiResponse({ 
    status: 200, 
    type: ProjectResponseDto, 
  ...
  async findOne(
    ...
    try {
      ...
      this.logger.error(`Failed to find project ${id} for user ${user.id}`, error);
      throw error;
    }
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Updates project name and/or description. Initial prompt is immutable.'
  })
  @ApiResponse({ 
    status: 200, 
    type: ProjectResponseDto, 
  ...
  async update(
    ...
    try {
      ...
      this.logger.error(`Failed to update project ${id} for user ${user.id}`, error);
      throw error;
    }
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Changes project status to ARCHIVED. Project remains accessible but hidden by default.'
  })
  @ApiResponse({ 
    status: 204, 
  ...
  async archive(
    ...
    try {
      ...
      this.logger.error(`Failed to archive project ${id} for user ${user.id}`, error);
      throw error;
    }
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Changes project status to DELETED. Project becomes inaccessible but remains in database for audit.'
  })
  @ApiResponse({ 
    status: 204, 
  ...
  async delete(
    ...
    try {
      ...
      this.logger.error(`Failed to delete project ${id} for user ${user.id}`, error);
      throw error;
    }
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Used by the orchestrator to update the list of generated files for a project'
  })
  @ApiResponse({ 
    status: 204, 
  ...
  async updateGeneratedFiles(
    ...
    try {
      ...
      await this.projectService.updateGeneratedFiles(id, updateDto.fileIds, updateDto.mode);
