...
describe('Cache E2E', () => {
  ...
  describe('Project Cache Workflow', () => {
    it(
      'should complete full project lifecycle with cache',
      async () => {
        ...
        expect(await cacheService.get(CACHE_KEYS.PROJECT(projectId))).toEqual(
          projectData,
        );
        expect(
          await cacheService.get(CACHE_KEYS.USER_PROJECTS_COUNT(userId)),
        ).toBe(1);
        expect(
          await cacheService.get(CACHE_KEYS.PROJECT_LIST(userId, 1, 10)),
        ).toEqual(projectList);
        ...
        expect(
          await cacheService.get(CACHE_KEYS.PROJECT(projectId)),
        ).toBeNull();
        expect(
          await cacheService.get(CACHE_KEYS.PROJECT_LIST(userId, 1, 10)),
        ).toBeNull();
        ...
        for (let i = 0; i < consultations; i++) {
          const projectFromCache = await cacheService.get(
            CACHE_KEYS.PROJECT(projectId),
          );
          expect(projectFromCache).toEqual(updatedProjectData);

          const statsFromCache = await cacheService.get(
            CACHE_KEYS.PROJECT_STATISTICS(projectId),
          );
        ...
        const archivedProject = await cacheService.get(
          CACHE_KEYS.PROJECT(projectId),
        );
        ...
        expect(
          await cacheService.get(CACHE_KEYS.PROJECT_STATISTICS(projectId)),
        ).toBeNull();
      },
      E2E_TIMEOUT,
    );
    ...
    it(
      'should handle multiple projects per user workflow',
      async () => {
        ...
        const cachedCount = await cacheService.get(
          CACHE_KEYS.USER_PROJECTS_COUNT(userId),
        );
        ...
        expect(
          await cacheService.get(CACHE_KEYS.PROJECT(projectToUpdate)),
        ).toBeNull();
        expect(
          await cacheService.get(CACHE_KEYS.PROJECT_LIST(userId, 1, 10)),
        ).toBeNull();
        ...
        expect(
          await cacheService.get(CACHE_KEYS.PROJECT('multi-project-1')),
        ).toBeTruthy();
        expect(
          await cacheService.get(CACHE_KEYS.PROJECT('multi-project-3')),
        ).toBeTruthy();
        ...
        expect(
          await cacheService.get(CACHE_KEYS.USER_PROJECTS_COUNT(userId)),
        ).toBeNull(); // Déjà supprimé par invalidateProjectCache précédent
      },
      E2E_TIMEOUT,
    );
  });
  ...
  describe('Statistics Cache Workflow', () => {
    it(
      'should handle statistics updates and aggregation workflow',
      async () => {
        ...
        const costUpdate = {
          costs: { claudeApi: 1.25, storage: 0.05, total: 1.3 },
        ...
        expect(stats.costs.total).toBe(1.3);
  ...
  describe('Export Cache Workflow', () => {
    it(
      'should handle export status caching workflow',
      async () => {
        ...
        await cacheService.set(
          CACHE_KEYS.EXPORT_STATUS(exportId),
          exportStartStatus,
          1800,
        ); // 30 min TTL
        ...
        for (let i = 0; i < 5; i++) {
          const polledStatus = await cacheService.get(
            CACHE_KEYS.EXPORT_STATUS(exportId),
          );
  ...
  describe('Rate Limiting Cache Workflow', () => {
    it(
      'should handle rate limiting with cache',
      async () => {
        ...
        let rateLimitData = await cacheService.get(
          CACHE_KEYS.RATE_LIMIT(userId, action),
        );
        ...
        for (let i = 2; i <= maxAttempts; i++) {
          ...
          rateLimitData = await cacheService.get(
            CACHE_KEYS.RATE_LIMIT(userId, action),
          );
  ...
  describe('Cache Performance and Monitoring', () => {
    it(
      'should demonstrate cache performance benefits',
      async () => {
    ...
    it(
      'should handle cache monitoring data collection',
      async () => {
        ...
        const hitRate =
          monitoringData.cacheHits /
          (monitoringData.cacheHits + monitoringData.cacheMisses);
