...
describe('Cache Integration', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      throw new Error(
        'Redis server is required for integration tests. Please start Redis first.',
      );
    }
  ...
  describe('Real Redis Connection', () => {
    ...
    it(
      'should retrieve Redis server info',
      async () => {
    ...
    it(
      'should use correct database for tests',
      async () => {
        const configService = app.get<ConfigService>(ConfigService);
        const config = getCacheConfig(configService);
    ...
    it(
      'should handle connection with authentication if configured',
      async () => {
  ...
  describe('Basic Operations', () => {
    it(
      'should perform complete set/get/delete cycle',
      async () => {
    ...
    it(
      'should handle multiple keys deletion',
      async () => {
  ...
  describe('Performance and Concurrency', () => {
    ...
    it(
      'should handle rapid cache invalidation',
      async () => {
  ...
  describe('Resilience and Error Handling', () => {
    it(
      'should handle key patterns correctly',
      async () => {
    ...
    it(
      'should handle Redis memory pressure gracefully',
      async () => {
    ...
    it(
      'should handle Unicode and special characters in keys and values',
      async () => {
        const specialCases = [
          {
          ...
          },
          {
            key: 'encoded:test',
            value: {
              data: 'Base64: ' + Buffer.from('test data').toString('base64'),
            },
          },
        ];
  ...
  describe('Business Logic Integration', () => {
    it(
      'should support complete project cache workflow',
      async () => {
        ...
        expect(await service.get(CACHE_KEYS.PROJECT(projectId))).toEqual(
          projectData,
        );
        expect(
          await service.get(CACHE_KEYS.PROJECT_STATISTICS(projectId)),
        ).toEqual(statisticsData);
        expect(
          await service.get(CACHE_KEYS.PROJECT_LIST(userId, 1, 10)),
        ).toEqual(projectListData);
        expect(await service.get(CACHE_KEYS.USER_PROJECTS_COUNT(userId))).toBe(
          2,
        );
        ...
        expect(
          await service.get(CACHE_KEYS.USER_PROJECTS_COUNT(userId)),
        ).toBeNull();
      },
      INTEGRATION_TIMEOUT,
    );
    ...
    it(
      'should handle user project cache isolation',
      async () => {
        ...
        await service.set(CACHE_KEYS.PROJECT_LIST(user1Id, 1, 10), [
          { id: project1Id },
        ]);
        await service.set(CACHE_KEYS.PROJECT_LIST(user2Id, 1, 10), [
          { id: project2Id },
        ]);
        ...
        expect(
          await service.get(CACHE_KEYS.PROJECT_LIST(user1Id, 1, 10)),
        ).toBeNull();

        // USER_PROJECTS_COUNT ne doit PAS être supprimé par invalidateUserProjectsCache
        expect(await service.get(CACHE_KEYS.USER_PROJECTS_COUNT(user1Id))).toBe(
          1,
        );

        // Les données du deuxième utilisateur doivent rester intactes
        expect(await service.get(CACHE_KEYS.PROJECT(project2Id))).toEqual({
          ownerId: user2Id,
        });
        expect(
          await service.get(CACHE_KEYS.PROJECT_LIST(user2Id, 1, 10)),
        ).toEqual([{ id: project2Id }]);
        expect(await service.get(CACHE_KEYS.USER_PROJECTS_COUNT(user2Id))).toBe(
          1,
        );
      },
      INTEGRATION_TIMEOUT,
  ...
  describe('Configuration Integration', () => {
    it(
      'should use configuration values correctly',
      async () => {
        const configService = app.get<ConfigService>(ConfigService);
        const config = getCacheConfig(configService);
    ...
    it(
      'should use key prefix from configuration',
      async () => {
        const configService = app.get<ConfigService>(ConfigService);
        const config = getCacheConfig(configService);
