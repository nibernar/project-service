...
function loadEnvironmentVariables(): void {
  ...
  if (process.env.DEBUG_TESTS === 'true') {
    ...
    console.log(
      '   TEST_DATABASE_URL:',
      process.env.TEST_DATABASE_URL ? 'configured' : 'missing',
    );
  }
}
...
async function createTestDatabase(config: DatabaseConfig): Promise<void> {
  ...
  try {
    ...
    console.error(
      `❌ Failed to create test database: ${getErrorMessage(error)}`,
    );
    throw error;
  } finally {
...
async function runMigrations(databaseUrl: string): Promise<void> {
  ...
  try {
    ...
    if (errorMsg.includes('P3005') && databaseUrl.includes('test')) {
      console.log(
        'ℹ️  Database not empty, attempting reset for test database...',
      );
      try {
...
async function checkRequiredServices(): Promise<void> {
  ...
  for (const service of services) {
    try {
      ...
      console.warn(
        `⚠️  ${service.name} is not available: ${getErrorMessage(error)}`,
      );
...
export default async function globalSetup(): Promise<void> {
  ...
  try {
    ...
    console.log(
      '   - Check that your .env.test file exists and contains TEST_DATABASE_URL',
    );
    console.log(
      '   - Verify that the database user has CREATE DATABASE permissions',
    );
