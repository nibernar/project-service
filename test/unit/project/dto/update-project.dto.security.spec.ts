  ...
  describe('XSS and HTML injection prevention', () => {
    ...
    it('should prevent script injection in description field', async () => {
      ...
      for (const payload of xssPayloads) {
        ...
        expect(descError?.constraints?.matches).toBe('description cannot contain HTML tags or potentially dangerous scripts');
      }
    });
    ...
    it('should prevent XML injection attempts', async () => {
      ...
      for (const payload of xmlPayloads) {
        ...
        expect(nameError).toBeDefined();
        expect(nameError?.constraints?.matches).toBe('name cannot contain potentially dangerous characters');
      }
    });
  ...
  describe('data corruption prevention', () => {
    ...
    it('should handle malformed objects safely', async () => {
      const malformedObjects = [
        ...
        { toString: () => { throw new Error('Bad toString'); } },
        // Object with throwing valueOf
        { valueOf: () => { throw new Error('Bad valueOf'); } },
        // Proxy that throws
        new Proxy({}, { 
          get: () => { throw new Error('Bad proxy'); },
          has: () => { throw new Error('Bad proxy'); }
        }),
      ];
    ...
    it('should handle memory exhaustion attempts', async () => {
      ...
      expect(() => {
        const testDto = Object.assign(new UpdateProjectDto(), memoryPressureData);
        validate(testDto);
      }).not.toThrow();
    });
  });
